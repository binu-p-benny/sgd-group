...
export async function submitBrochureRequest(prevState, formData) {
  const name = readField(formData, 'name');
  const email = readField(formData, 'email');
  const phone = readField(formData, 'phone');
  ...
  try {
    const { error } = await supabaseAdmin()
      .from('submissions')
      .insert({ type: 'brochure', name, email, phone });

    if (error) {
      console.error('submitBrochureRequest:', error);
      return { error: GENERIC_ERROR };
    }

    return { success: true };
  } catch (err) {
    console.error('submitBrochureRequest:', err);
    return { error: GENERIC_ERROR };
  }
}
