...
export async function permanentlyDeleteSubmission(formData) {
  if (!(await isAdminAuthenticated())) {
    redirect('/admin/login');
  }
  ...
  // "Permanent" only means gone from the admin UI (Active and Hidden).
  // We never run a SQL DELETE here — the row stays recoverable via the
  // Supabase dashboard if it's ever needed again.
  const { error } = await supabaseAdmin()
    .from('submissions')
  ...
  if (error) {
    console.error('permanentlyDeleteSubmission:', error);
  }
